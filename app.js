...
function renderMoneyTags() {
  $$("[data-money]").forEach((node) => {
    node.textContent = money(Number(node.dataset.money));
  });
}
...
async function loadUserData(shouldRender = true) {
  ...
  const [debts, budget, transactions, goals, members, incomes] = await Promise.all([
    ...
    supabaseClient.from("budget_categories").select("id,name,category,planned,spent,due_day,is_recurring,frequency,notes,month_key").or(`month_key.eq.${state.selectedMonth},month_key.is.null`).order("created_at"),
...
function totals() {
  ...
  const monthlyExpenses = state.budget.reduce((sum, item) => sum + monthlyExpenseAmount(normalizeExpense(item)), 0);
...
function stat(label, value, className, status) {
  return `<article class="stat"><small>${label}</small><strong>${value}</strong><span class="${className}">${status}</span></article>`;
}
...
function renderStrategyLegacy() {
  ...
  return `
    <div class="split">
      <section class="panel">
        <span class="chip">${t("snowball")}</span>
        <h3>${state.lang === "fr" ? "Motivation rapide" : "Fast motivation"}</h3>
        <p>${state.lang === "fr" ? "Remboursez d’abord le plus petit solde pour créer de l’élan." : "Pay the smallest balance first to build momentum."}</p>
        ${strategyList(snowball)}
      </section>
      <section class="panel">
        <span class="chip">${t("avalanche")} · ${t("recommended")}</span>
        <h3>${state.lang === "fr" ? "Moins d’intérêts" : "Less interest"}</h3>
        <p>${state.lang === "fr" ? "Remboursez d’abord le taux le plus élevé pour réduire le coût total." : "Pay the highest rate first to reduce total cost."}</p>
        ${strategyList(avalanche)}
      </section>
    </div>
  `;
}
...
function strategyList(items) {
  return `<ol>${items.map((item) => `<li><strong>${item.name}</strong> - ${money(item.balance)} · ${item.rate.toFixed(2)}%</li>`).join("")}</ol>`;
}
...
function activeLabel(user) {
  const fr = state.lang === "fr";
  if (user.isSuspended) return fr ? "Inactif" : "Inactive";
  return fr ? "Actif" : "Active";
}

function subscriptionStatusLabel(user) {
  const base = activeLabel(user);
...
function renderAdmin() {
  ...
  const start = state.admin.total ? ((state.admin.page - 1) * state.admin.pageSize) + 1 : 0;
  const end = Math.min(state.admin.total, state.admin.page * state.admin.pageSize);
  return `
    ...
    <section class="panel">
        ...
        <table class="admin-table">
          <thead>
            <tr>
              ...
              <th>${fr ? "Expiration" : "Expiration"}</th>
              <th>${fr ? "Date création" : "Created"}</th>
      ...
      <div class="admin-pagination">
        <span>${fr ? "Résultats" : "Results"} ${start}-${end} / ${state.admin.total}</span>
        <div>
          <button class="secondary-button" data-admin-page="prev" type="button" ${state.admin.hasPrevious ? "" : "disabled"}>${fr ? "Précédent" : "Previous"}</button>
          <button class="secondary-button" data-admin-page="next" type="button" ${state.admin.hasNext ? "" : "disabled"}>${fr ? "Suivant" : "Next"}</button>
        </div>
      </div>
    </section>
...
async function adminLoadUsers(page = state.admin.page) {
  const params = new URLSearchParams({
    q: state.admin.query || "",
    page: String(page),
    pageSize: String(state.admin.pageSize || 20)
  });
  const response = await authFetch(`/api/admin/users?${params.toString()}`);
  const result = await response.json();
  if (!response.ok) throw new Error(result.detail || result.error || "users_list_failed");
  state.admin.users = result.users || [];
  state.admin.page = result.page || page;
  state.admin.pageSize = result.pageSize || 20;
  state.admin.total = result.total || 0;
  state.admin.hasPrevious = Boolean(result.hasPrevious);
  state.admin.hasNext = Boolean(result.hasNext);
  state.admin.loaded = true;
}
...
function bindViewActions() {
  ...
  $$("[data-admin-page]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const nextPage = direction === "next" ? state.admin.page + 1 : state.admin.page - 1;
      if (nextPage < 1) return;
      state.admin.message = "";
      try {
        await adminLoadUsers(nextPage);
      } catch (error) {
        state.admin.message = error.message;
      }
      renderView();
  ...
  if (settingsLang) {
    ...
    settingsLang.addEventListener("change", (event) => updatePreference("lang", event.target.value));
